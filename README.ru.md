...
|---|---|---|---|---|
| Open source | GPL-3.0 (клиенты), AGPL-3.0 (сервер) | нет | MIT | MIT |
...
| Работает без аккаунта | да | частично — только локально; sync и AI требуют аккаунт | да | да |
| Шифрованный vault | да, всегда (Argon2id + XChaCha20-Poly1305) | да | нет | опционально, по желанию |
| Синхронизация | свой self-hosted сервер, zero-knowledge | облако вендора, E2E (платно) | нет | синк конфига через self-hosted Tabby Web (E2E опционально) |
| Шаринг в командах | да, E2E | платный тариф | нет | нет |
| SFTP | двухпанельный UI | да | только командная строка (`psftp`) | встроенная панель |
| Port forwarding | local, remote, dynamic | да | да | да |
| Serial / Telnet | да / да | да / да | да / да | да / да |
| Mosh | нет | да | нет | нет |
| AI-ассистент | опционально: BYOK-облако или полностью локальная модель | да, на стороне облака; нужен аккаунт | нет | нет |
...
- **Сниппеты и AI** — библиотека команд с type-ahead в терминале; AI-ассистент с per-host
...
Полный детальный список возможностей — в **[docs/FEATURES.md](docs/FEATURES.md)**.
